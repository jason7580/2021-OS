...
//  The OpenAFile function is used for kernel open system call
	...
	if(!OpenFileTable[i]) return -1;
        return i;
    }
    int CloseFile(OpenFileId id){
	if(id < 0 || id > 19) return -1;
	if(!OpenFileTable[id]) return -1;
	...
	if(id < 0 || id > 19 || size < 0) return -1;
	if(OpenFileTable[id] != NULL){
		int r = OpenFileTable[id]->Write(buffer, size);
    	if (r < 0) return -1;   
		return r;
	}
	return -1;
	...
	if(id < 0 || id > 19 || size < 0) return -1;
    if(OpenFileTable[id] != NULL){
		int r = OpenFileTable[id]->Read(buffer, size);
        if (r < 0) return -1;   
		return r;
	}
    return -1;
