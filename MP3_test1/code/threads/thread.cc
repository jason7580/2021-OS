...
Thread::Yield ()
{
    ...
    // MP3
    this->burst += kernel->stats->totalTicks - this->start;
    this->cmptime = max(0.0, this->predict - this->burst);
...
Thread::Sleep (bool finishing)
{
    ...
    // MP3
    this->burst += (kernel->stats->totalTicks - this->start);
    DEBUG('z', "[D] Tick [" << kernel->stats->totalTicks << "]: Thread [" << this->getID() << "] update approximate burst time, \
from: [" << this->predict << "], add [" << this->burst << "], to [" << 0.5 * (this->burst + this->predict) << "]");
    this->predict = 0.5 * (this->burst + this->predict);
    //this->burst = 0;
    this->cmptime = this->predict;
